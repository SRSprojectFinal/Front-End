...
async function updateGrade(gradeType, email, grade) {
  const endpoint = `http://localhost:8080/cursos/frontend/atualizar/${gradeType}`;
...
async function finalizarTurma() {
  ...
  const resp = await fetch("http://localhost:8080/cursos/frontend/remover-todos", { method: "DELETE" });
  if (resp.ok) {
