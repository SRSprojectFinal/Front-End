...
async function updateGrade(gradeType, email, grade) {
  const endpoint = `http://localhost:8080/cursos/programming/atualizar/${gradeType}`;
  ...
  for (const aluno of studentScheduleDb) {
    ...
    await fetch("http://localhost:8080/historico/adicionar", {
      ...
      body: JSON.stringify({
        ...
        curso: "Programming-Basis",
        situacao: situacao
      })
    });
  }

  const resp = await fetch("http://localhost:8080/cursos/programming/remover-todos", { method: "DELETE" });
  if (resp.ok) {
