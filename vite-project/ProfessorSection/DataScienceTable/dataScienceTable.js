...
async function fetchStudentsFromBackend() {
  try {
    const response = await fetch('http://localhost:8080/cursos/datascience/alunos');
    if (!response.ok) {
...
async function updateGrade(gradeType, email, grade) {
  const endpoint = `http://localhost:8080/cursos/datascience/atualizar/${gradeType}`;
...
async function finalizarTurma() {
  ...
  const resp = await fetch("http://localhost:8080/cursos/datascience/remover-todos", { method: "DELETE" });
  if (resp.ok) {
