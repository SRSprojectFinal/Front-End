...
async function fetchStudentsFromBackend() {
  try {
    const response = await fetch('http://localhost:8080/cursos/uiux/alunos');
    if (!response.ok) {
...
async function updateGrade(gradeType, email, grade) {
  const endpoint = `http://localhost:8080/cursos/uiux/atualizar/${gradeType}`;
...
async function finalizarTurma() {
  ...
  const resp = await fetch("http://localhost:8080/cursos/uiux/remover-todos", { method: "DELETE" });
  if (resp.ok) {
