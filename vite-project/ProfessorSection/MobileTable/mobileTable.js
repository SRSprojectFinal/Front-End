...
async function fetchStudentsFromBackend() {
  try {
    const response = await fetch('http://localhost:8080/cursos/mobile/alunos');
    if (!response.ok) {
...
async function updateGrade(gradeType, email, grade) {
  const endpoint = `http://localhost:8080/cursos/mobile/atualizar/${gradeType}`;
...
async function finalizarTurma() {
  ...
  const resp = await fetch("http://localhost:8080/cursos/mobile/remover-todos", { method: "DELETE" });
  if (resp.ok) {
